    ...
    public void favoriteAdd(final CategoryItemData item) {
        MainDBHelper.getSingleton().favoriteAdd(item.category, item.getFavoriteKey(), item.jsonItem.toString(), new DBExecuteListener() {
            ...
            public void finished(long result) {
                ...
                BSApplication.defaultNotificationCenter.notifyOnMainThread(NotificationEvent.CATEGORY_ITEM_DATA_FINISHED, item);
            }
        });
    }
    ...
    public void favoriteMore() {
        if (nextID < 0) {
            new Handler().post(new Runnable() {
                @Override
                public void run() {
                    BSApplication.defaultNotificationCenter.notifyOnMainThread(NotificationEvent.FAVORITE_QUERYLIST_FINISHED, new ArrayList<CategoryItemData>());
                }
            });
            return;
        }
        MainDBHelper.getSingleton().favoriteQuery(LOAD_MORE_COUNT + 1, nextID, new DBQueryListener() {
            ...
            public void finished(Cursor cursor) {
                ...
                if (cursor == null) {
                    BSApplication.defaultNotificationCenter.notifyOnMainThread(NotificationEvent.FAVORITE_QUERYLIST_FINISHED, null);
                    return;
                }
                ...
                BSApplication.defaultNotificationCenter.notifyOnMainThread(NotificationEvent.FAVORITE_QUERYLIST_FINISHED, itemList);
